...
const CardVert: React.FC<IProps> = ({ blog, ispromoted, category, }) => {
  ...
  return (
    <div className={`card border-0 bg-${isdarkMode?'dark':'light'}`} >
      ...
      <div className={`card-footer text-muted d-flex justify-content-between py-3 border-${isdarkMode?'white':'black'} bg-${isdarkMode?'dark':'light'}`} >
      </div>
    </div >
