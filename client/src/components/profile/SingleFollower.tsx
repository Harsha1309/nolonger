            ...
            <div className="d-flex flex-row justify-content-between align-items-center ">
                <Link to={`/profile/${follower._id}`} className="text-decoration-none">
                    <div className="d-flex flex-row align-items-center"><img className="rounded-circle" src={follower.avatar} width="55" height="55" />
                        <div className="d-flex flex-column align-items-start ml-2"><span className="font-weight-bold mx-2">{follower.name}<Onlytick role={follower.role} /></span><span className={`followers mx-2 text-${isdarkMode?'light':'dark'}`}>{follower.follower.length} Followers</span></div>
                    </div>
                </Link>
